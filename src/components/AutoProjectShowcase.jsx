...
const CATEGORY_LABELS = {
  'production': 'Signature Engineering',
            ...
            <div className="flex flex-wrap justify-center gap-4 mb-12">
              {categories.map((catKey) => (
                <button
                  ...
                  className={`px-6 py-3 rounded-full transition-all duration-300 ${selectedCategory === catKey
                      ? 'bg-amber-500 text-black font-semibold'
                      : 'bg-white/10 text-gray-300 hover:bg-white/20'
                    }`}
                >
                  {catKey === 'All' ? 'All' : CATEGORY_LABELS[catKey]}
                  {catKey !== 'All' && ` (${projectsData.filter(p => p.category === catKey).length})`}
                </button>
              ))}
            </div>
          </TextReveal>
