      ...
      <div className="max-w-6xl mx-auto px-6 relative z-10">
        ...
        <div className="text-center mb-20">
          <div className="inline-block relative">
            <h2 className={`text-6xl lg:text-7xl font-black mb-6 text-transparent bg-clip-text ${
              isSpace 
          ...
          {timelineData.map((item, index) => {
            ...
            return (
              <div
                key={index}
                data-index={index}
                className={`timeline-item relative flex items-center mb-20 transition-all duration-1000 ${
              ...
              >
                ...
                <div className={`absolute left-1/2 transform -translate-x-1/2 w-12 h-12 rounded-full z-20 transition-all duration-500 ${
                  ...
                  <div className={`w-full h-full bg-gradient-to-r ${item.color} rounded-full shadow-2xl flex items-center justify-center border-4 border-white/20`} style={{
                    ...
                    <span className="text-white text-lg font-bold drop-shadow-lg">{item.icon}</span>
                  </div>
                  {isActive && (
                    <>
                ...
                <div className={`w-5/12 ${isLeft ? 'pr-16' : 'pl-16'} transition-all duration-700 ${
                  ...
                  <div className={`relative group cursor-pointer transition-all duration-700 ${
                    ...
                    <div className="relative bg-black/40 backdrop-blur-xl border border-white/10 rounded-3xl p-8 shadow-2xl hover:shadow-3xl transition-all duration-700" style={{
                      background: 'linear-gradient(135deg, rgba(255,255,255,0.1) 0%, rgba(255,255,255,0.05) 100%)',
                      backdropFilter: 'blur(20px)',
                      border: '1px solid rgba(255,255,255,0.2)'
                    }}>
                      {/* Holographic year badge */}
                      <div className={`inline-flex items-center gap-3 mb-6 px-6 py-3 bg-gradient-to-r ${item.color} text-white rounded-full font-bold shadow-2xl`} style={{
                        boxShadow: `0 0 20px ${item.glow}60`
                      }}>
                        <span className="text-xl drop-shadow-lg">{item.icon}</span>
                        <span className="text-lg tracking-wider">{item.year}</span>
                      </div>

                      {/* Glowing title */}
                      <h3 className={`text-3xl font-bold mb-4 text-transparent bg-clip-text bg-gradient-to-r ${item.color}`} style={{
                      ...
                      <div className="flex items-center gap-3">
                        <span className={`inline-block px-4 py-2 text-sm font-bold rounded-full border-2 ${
                      ...
                      <div className={`absolute ${isLeft ? 'right-0' : 'left-0'} top-1/2 transform -translate-y-1/2 w-0 h-1 bg-gradient-to-r ${item.color} transition-all duration-700 group-hover:w-12 opacity-80`} style={{
                ...
                <div className={`absolute left-1/2 transform -translate-x-1/2 ${isLeft ? 'translate-x-6' : '-translate-x-6'} w-12 h-1 bg-gradient-to-r ${item.color} opacity-70`} style={{
        ...
        <div className="mt-20 text-center">
          <div className={`inline-flex items-center gap-4 px-8 py-4 backdrop-blur-xl rounded-full shadow-2xl ${
            isSpace 
